...
export async function fetchNotionPosts() {
  ...
  let response = await fetch(`${notionWorkerUrl}${notionBlogTableId}`)
  
  return await response.json();
}

export async function fetchNotionPost(id) {
  const notionWorkerUrl = "https://api.bangarang.dev/v1/page/";

  let response = await fetch(`${notionWorkerUrl}${id}`)
